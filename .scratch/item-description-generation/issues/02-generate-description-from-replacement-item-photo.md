...
- [x] **Save & generate description** is enabled when the edit draft contains an unsaved replacement Item Photo and disabled again if that photo is removed.
- [x] The background request captures the visible replacement photo and unsaved Description together with every other editable draft field.
- [x] A single immutable photo revision is allocated before optimistic presentation and supplies the full and thumbnail locations used by the captured Item.
- [x] The background workflow unconditionally saves the complete captured Item with the new revision locations before transferring photo bytes.
...
- [x] Controller tests verify replacement-photo eligibility, immediate close, and optimistic presentation of the new immutable photo revision.
- [x] Storage and Firestore emulator tests authorize the new immutable revision and captured Item update for a Household Member while denying cross-Household access.
